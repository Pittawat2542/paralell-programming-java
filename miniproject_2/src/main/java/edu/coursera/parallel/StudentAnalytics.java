...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        return Arrays.stream(studentArray).parallel().filter(student -> student.checkIsCurrent()).mapToDouble(student -> student.getAge()).average().getAsDouble();
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        return (int) Arrays.stream(studentArray).parallel().filter(st -> !st.checkIsCurrent() && st.getAge() > 20 && st.getGrade() < 65).count();
    }
}
